...
import { useStoreState } from "../lib/zustand/store";
...
export default function ImportResume() {
    ...
    useEffect(() => {
       const hasData = Boolean(
        storeState.resume.profile.name ||
        storeState.resume.workExperiences[0]?.company ||
        storeState.resume.educations[0]?.college ||
        storeState.resume.projects[0]?.project ||
        storeState.resume.skills.descriptions.length > 0 ||
        storeState.resume.custom.descriptions.length > 0
       );
       setHasUsedAppBefore(hasData);
    }, [storeState]);
